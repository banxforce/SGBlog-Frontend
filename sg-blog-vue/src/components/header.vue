...
<!--						<el-menu-item index="/Reward"><i class="fa fa-wa fa-cny"></i> 赞赏</el-menu-item>-->
	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':headBg}">
		...
		<div class="h-information">
                    ...
                    <img :src="this.$store.state.themeObj.head_portrait?this.$store.state.themeObj.head_portrait:headAvatar" alt="">

			<h2 class="h-description">

                        {{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:headText}}
...
export default {
  data() {
    ...
    return {
      ...
      // path:'',//当前打开页面的路径
      input: "", //input输入内容
      headBg: "url(static/img/head-bg.webp)", //头部背景图 需要在style中引入，所以要加上url('')
      headAvatar: "static/img/head-avatar.jpg", //头像
      headText: "短暂的总是浪漫，漫长总会不满", //头像下方的文字
      projectList: "" //项目列表
    };
  },
  methods: {
